<template>
  <div class="edit-recipe-page">
    <v-container>
      <v-row class="mb-6">
        <v-col cols="12">
          <div class="page-header">
            <h1 class="text-h3 font-weight-bold mb-2">Editar Receita</h1>
            <p class="text-h6 text-medium-emphasis">
              Atualize sua receita com novas informações
            </p>
          </div>
        </v-col>
      </v-row>

      <v-row v-if="isLoadingRecipe">
      ...
      <v-row v-else-if="!currentRecipe">
        <v-col cols="12" class="text-center">
          <v-icon icon="mdi-alert-circle" size="64" color="error" class="mb-4" />
          <h3 class="text-h5 mb-2">Receita não encontrada</h3>
          <p class="text-body-1 mb-4">A receita que você está tentando editar não existe.</p>
          <v-btn :to="'/recipes'" color="primary">Voltar para Receitas</v-btn>
        </v-col>
      </v-row>
...
import type { UpdateRecipeData } from '@/types'
...
const formData = ref<UpdateRecipeData>({
  nome: '',
  ingredientes: '',
  modo_preparo: '',
...
const loadRecipe = async () => {
  ...
  try {
    const result = await recipesStore.fetchRecipeById(recipeId.value)
    
    if (result.success && currentRecipe.value) {
      formData.value = {
    ...
    addNotification({
      type: 'error',
      title: 'Erro',
      message: 'Erro ao carregar a receita'
    })
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    const result = await recipesStore.updateRecipe(recipeId.value, formData.value)
